...
public class MeshSceneLoader implements FileLoader<MeshScene>
{
	...
	public Resource<MeshScene> loadFile(File file)
	{
		...
		// Load each mesh in the scene
		ArrayList<UncompiledMesh> uncompiledMeshes = new ArrayList<>();
		for (int sceneId = 0; sceneId < scene.mNumMeshes(); sceneId++)
		{
			...
			// TODO If mesh has larger than 65k vertices, load as int array, not
			// short array.
			...
			if (boneCount > 0)
			{
				...
				uncompiledMeshes
						.add(new UncompiledMesh(mesh.mName().dataString(), vertexData, skeleton));
			}
			else
			{
				int[] attributes = new int[]
				...
				VertexData vertexData = new VertexData(vertices, triangles, attributes);
				uncompiledMeshes
						.add(new UncompiledMesh(mesh.mName().dataString(), vertexData, null));
			}
		}
