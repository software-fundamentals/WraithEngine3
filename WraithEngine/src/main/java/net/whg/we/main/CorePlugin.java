...
public class CorePlugin extends BasePlugin
{
	...
	void loadPluginsFromFile()
	{
		...
		for (File file : folder.listFiles())
		{
			...
			if (file.isDirectory())
			{
				Log.warnf("File '%s' is in plugin folder, but is a directory!",
						fileName);
				continue;
			}
			...
			{
				Log.warnf(
						"File '%s' is in plugin folder, but is not cannot be read!",
						fileName);
				continue;
			}
			...
			{
				Log.warnf(
						"File '%s' is in plugin folder, but is not a jar file",
						fileName);
				continue;
			}
