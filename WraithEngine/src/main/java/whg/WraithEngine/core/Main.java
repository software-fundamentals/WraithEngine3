...
public class Main
{
	...
	public static void main(String[] args)
	{
		...
		FirstPersonCamera firstPersonCamera =
				new FirstPersonCamera(gameLoop.getCamera());
		...
		world.addEntity(new InitalizerEntity(world)
		{
			...
			public void run()
			{
				...
				world.getCamera().getLocation()
						.setPosition(new Vector3f(0f, 3f, 5f));

				File monkeyHeadModel =
						FileUtils.getResource(null, "monkey_head.fbx");
				...
				SkinnedModel monkeyModel =
						new SkinnedModel(monkey, new Location(), boneMaterial);
				Model floorModel =
						new Model(floor, new Location(), baseMaterial);

				world.addEntity(monkeyModel);
				world.addEntity(floorModel);

				Quaternionf rot =
						new Quaternionf().rotateLocalX((float) (-Math.PI / 2f));
				monkeyModel.getLocation().setRotation(rot);
				floorModel.getLocation().setRotation(rot);

				monkey.getSkeleton().getBones()[1].getLocalTransform()
						.rotateLocalZ(1f);
